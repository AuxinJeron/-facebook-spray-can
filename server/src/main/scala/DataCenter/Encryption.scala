...
class RSA(val keySize: Int) {
  ...
  def sign(message: Array[Byte]): Array[BigInt] = {
    message.map(x => Array[Byte](x)).map(si)
  }

  def verify(cipher: Array[BigInt]): Array[Byte] = {
    cipher.map(ver).map(_.toByteArray).flatMap(x => x)
  }

  def si(message: Array[Byte]): BigInt = {
    BigInt(message).modPow(privateKey, n)
  }

  def ver(cipher: BigInt): BigInt = {
    cipher.modPow(publicKey, n)
  }
